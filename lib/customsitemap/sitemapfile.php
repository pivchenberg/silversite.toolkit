 ...
 */
namespace Silversite\Toolkit\CustomSitemap;
	...
	extends File
{
	...
	public function addEntry($entry)
	{
		...
		else
		{
			...
			$fileUrl = $entry['XML_LOC'];
			$domain = $this->settings['PROTOCOL'].'://'.\CBXPunycode::toASCII($this->settings['DOMAIN'], $e = null);
			$fileUrl = str_replace($domain, "", $fileUrl);
			$urlDepthLevel = count(array_filter(explode("/", $fileUrl), function($v){ if(!empty($v)) return $v;} ));
			$entry['PRIORITY'] = $this->getPriority($urlDepthLevel);
	...
	public function addFileEntry(File $f)
	{
		if($f->isExists() && !$f->isSystem())
		{
			$this->addEntry(array(
				...
				'XML_LASTMOD' => date('Y-m-d', $f->getModificationTime())
			));
		}
	}
	...
	public function addIBlockEntry($url, $modifiedDate)
	{
		$this->addEntry(array(
			...
			'XML_LASTMOD' => date('Y-m-d', $modifiedDate - \CTimeZone::getOffset()),
		));
	}
	...
	protected function getPriority($urlDepthLevel)
	{
		switch($urlDepthLevel)
		{
			case 0:
				$priority = 1;
				break;
