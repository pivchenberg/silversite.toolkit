...
Class SimilarOffer {
	...
	{
		$this->arLW = $arLW;
		$startLW = $lengthValue."x".$widthValue;
		if(!in_array($startLW, $arLW))
			$this->startLWValue = "200x160";
		else
			$this->startLWValue = $startLW;
		$this->direction = $direction;
		$this->LWKey = array_search($this->startLWValue, $this->arLW);
	}

	public function getSimilarOffer($arOffers)
	{
		...
		if(isset($arOffers[$lwKeyValue]))
			return $arOffers[$lwKeyValue];
		elseif($this->plusEnded && $this->minusEnded)
			//Не удалось ничего найти
			return 0;
		else
			return $this->getSimilarOffer($arOffers, $LWKey);
	}
