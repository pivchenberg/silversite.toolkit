...
Main\Loader::includeModule("silversite.toolkit");
...
while($sitemap = $data->NavNext())
{
	...
	$row = &$adminList->AddRow($sitemap["ID"], $sitemap, "seo_sitemap_edit_custom.php?ID=".$sitemap["ID"]."&lang=".LANGUAGE_ID, Loc::getMessage("SITEMAP_EDIT_TITLE"));
	...
	$row->AddField("NAME", '<a href="seo_sitemap_edit_custom.php?ID='.$sitemap["ID"].'&amp;lang='.LANGUAGE_ID.'" title="'.Loc::getMessage("SITEMAP_EDIT_TITLE").'">'.Converter::getHtmlConverter()->encode($sitemap['NAME']).'</a>');
	...
	$row->AddActions(array(
		array(
			...
			"ACTION" => $adminList->ActionRedirect("seo_sitemap_edit_custom.php?ID=".$sitemap["ID"]."&lang=".LANGUAGE_ID),
			"DEFAULT" => true,
		),
		array(
			"ICON" => "move",
...
foreach($arSites as $arRes)
{
	$arDDMenu[] = array(
		...
		"LINK" => "seo_sitemap_edit_custom.php?lang=".LANGUAGE_ID."&site_id=".$arRes['LID']
	);
}
...
BX.runSitemap = function(ID, value, pid, NS)
{
	...
	BX.ajax.post('/bitrix/admin/seo_sitemap_run_custom.php', {
		lang:'<?=LANGUAGE_ID?>',
		action: 'sitemap_run',
		ID: ID,
		value: value,
...
{
	window.tbl_sitemap.GetAdminList('/bitrix/admin/seo_sitemap_custom.php?lang=<?=LANGUAGE_ID?>');
};
</script>
