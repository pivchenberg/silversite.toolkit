...
if($ID > 0)
{
	...
	else
	{
		if($_REQUEST['action'] == 'delete' && check_bitrix_sessid())
		{
			...
			LocalRedirect(BX_ROOT."/admin/seo_sitemap_custom.php?lang=".LANGUAGE_ID);
		}
...
if($_SERVER['REQUEST_METHOD'] == 'POST' && check_bitrix_sessid() && (strlen($_POST["save"]) > 0 || strlen($_POST['apply']) > 0 || strlen($_POST['save_and_add']) > 0))
{
	...
	if(empty($errors))
	{
		...
		if($result->isSuccess())
		{
			...
			{
				LocalRedirect(BX_ROOT."/admin/seo_sitemap_custom.php?lang=".LANGUAGE_ID."&run=".$ID."&".bitrix_sessid_get());
			}
			else
			{
				LocalRedirect(BX_ROOT."/admin/seo_sitemap_edit_custom.php?lang=".LANGUAGE_ID."&ID=".$ID."&".$tabControl->ActiveTabParam());
			}
		}
		else
		{
...
$aMenu[] = array(
	...
	"LINK"	=> "/bitrix/admin/seo_sitemap_custom.php?lang=".LANGUAGE_ID,
	"ICON"	=> "btn_list",
...
if ($ID > 0)
{
	$aMenu[] = array(
		...
		"LINK"	=> "javascript:if(confirm('".Loc::getMessage("SITEMAP_DELETE_CONFIRM")."')) window.location='/bitrix/admin/seo_sitemap_edit_custom.php?action=delete&ID=".$ID."&lang=".LANGUAGE_ID."&".bitrix_sessid_get()."';",
		"ICON"	=> "btn_delete",
